...
func testReleaseCandidate() model.ReleaseCandidate {
	return model.ReleaseCandidate{Provider: "github-actions", Repository: "owner/repo", RepositoryID: "1", OwnerID: "2", RunID: "3", RunAttempt: "1", WorkflowRef: "owner/repo/.github/workflows/release.yml@" + strings.Repeat("c", 40), WorkflowSHA: strings.Repeat("c", 40), SignerWorkflowRef: "owner/repo/.github/workflows/release.yml@" + strings.Repeat("c", 40), SignerWorkflowSHA: strings.Repeat("c", 40), Ref: "refs/heads/main", Attestation: model.ReleaseAttestationIdentity{Issuer: githubActionsOIDCIssuer, SubjectDigest: "sha256:" + strings.Repeat("b", 64), MaterialSHA: strings.Repeat("a", 40)}}
}
