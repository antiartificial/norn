...
var accessTokenScopes = map[string]struct{}{
	...
	ScopeReleaseStage: {}, ScopeReleaseQualify: {}, ScopeReleasePromote: {}, ScopeReleaseRollback: {}, ScopeFleetOperate: {},
}
...
// CIIdentity is copied from a verified GitHub OIDC token into Norn's own
// short-lived token. Release request data is compared against these claims,
// never trusted merely because a caller repeated them in JSON.
type CIIdentity struct {
	...
	JobWorkflowSHA       string `json:"jobWorkflowSha,omitempty"`
	Ref                  string `json:"ref"`
