...
func (h *Handler) validateGitHubActionsAssertion(r *http.Request, raw string) (*githubActionsClaims, error) {
	...
	keys, err := fetchGitHubJWKs(r, h.cfg.GitHubActionsOIDCJWKSURL)
	if err != nil {
...
func fetchGitHubJWKs(r *http.Request, rawURL string) (map[string]interface{}, error) {
	parsed, err := url.Parse(rawURL)
	if err != nil || parsed.Scheme != "https" || parsed.Hostname() != "token.actions.githubusercontent.com" || parsed.Path != "/.well-known/jwks" {
		return nil, fmt.Errorf("GitHub Actions JWKS URL must be the fixed HTTPS GitHub issuer JWKS")
	}
	...
	if len(keys) == 0 {
		return nil, fmt.Errorf("JWKS has no supported public keys")
	}
	return keys, nil
}
...
func (h *Handler) authorizeGitHubActionsClaims(c *githubActionsClaims, request githubActionsExchangeRequest) (CIIdentity, error) {
	...
	if request.Scope == ScopeFleetOperate {
		...
		if c.RepositoryVisibility != "public" || !matchesRepository(c, h.cfg.GitHubActionsAllowedRepositories) || !matchesAny(request.App, h.cfg.GitHubActionsAllowedApps) || !matchesAny(c.Environment, h.cfg.GitHubActionsAllowedEnvironments) || !matchesAny(c.JobWorkflowRef, h.cfg.GitHubActionsAllowedWorkflowRefs) || !fullSourceSHAPattern.MatchString(c.JobWorkflowSHA) {
