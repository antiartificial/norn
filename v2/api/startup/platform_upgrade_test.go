...
[[ -z "${NORN_API_TOKEN:-}" ]]
printf '%s\n' '{"name":"norn.startup/v2","schemaModes":["auto","check","migrate-only"],"startupModes":["active","passive"],"passiveRoutes":["/api/health","/api/version","/api/schema"],"schemaContract":{"readerVersion":2,"writerVersion":11,"catalogMigrationVersion":14,"catalogMinimumReaderVersion":2,"catalogMinimumWriterVersion":11}}'
`)
