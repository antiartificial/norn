...
[[ -z "${NORN_API_TOKEN:-}" ]]
printf '%s\n' '{"name":"norn.startup/v2","schemaModes":["auto","check","migrate-only"],"startupModes":["active","passive"],"passiveRoutes":["/api/health","/api/version","/api/schema"],"schemaContract":{"readerVersion":4,"writerVersion":26,"catalogMigrationVersion":34,"catalogMinimumReaderVersion":4,"catalogMinimumWriterVersion":26}}'
`)
