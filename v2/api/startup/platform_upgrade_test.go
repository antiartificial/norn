...
[[ -z "${NORN_API_TOKEN:-}" ]]
printf '%s\n' '{"name":"norn.startup/v2","schemaModes":["auto","check","migrate-only"],"startupModes":["active","passive"],"passiveRoutes":["/api/health","/api/version","/api/schema"],"schemaContract":{"readerVersion":5,"writerVersion":31,"catalogMigrationVersion":41,"catalogMinimumReaderVersion":5,"catalogMinimumWriterVersion":31}}'
`)
