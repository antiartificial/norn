...
[[ -z "${NORN_API_TOKEN:-}" ]]
printf '%s\n' '{"name":"norn.startup/v2","schemaModes":["auto","check","migrate-only"],"startupModes":["active","passive"],"passiveRoutes":["/api/health","/api/version","/api/schema"],"schemaContract":{"readerVersion":2,"writerVersion":13,"catalogMigrationVersion":16,"catalogMinimumReaderVersion":2,"catalogMinimumWriterVersion":13}}'
`)
