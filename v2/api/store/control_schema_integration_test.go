...
func TestControlSchemaAdoptsLegacyRowsWithoutReplacingEvidence(t *testing.T) {
	...
	if len(migrations) != 38 || migrations[16].Version != 17 {
		t.Fatalf("control migration catalog has %d migrations, want immutable baseline through 17 and appended migrations through 38", len(migrations))
	}
	...
	if status.CurrentMigrationVersion != 38 || len(status.AppliedVersions) != 38 || status.AppliedVersions[37] != 38 || status.MinimumReaderVersion != MySQLRetainedArtifactReaderVersion || status.MinimumWriterVersion != MySQLRestoreRecoveryReleaseWriterVersion {
