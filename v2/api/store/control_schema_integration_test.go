...
func TestControlSchemaAdoptsLegacyRowsWithoutReplacingEvidence(t *testing.T) {
	...
	if len(migrations) != 8 || migrations[0].Version != 1 || migrations[1].Version != 2 || migrations[2].Version != 3 || migrations[3].Version != 4 || migrations[4].Version != 5 || migrations[5].Version != 6 || migrations[6].Version != 7 || migrations[7].Version != 8 {
		t.Fatalf("control migrations = %#v, want immutable baseline 1 followed by acceptance 2, effects 3, checkpoints 4, database catalog 5, evidence archive 6, archive reader contract 7 and evidence reserve 8", migrations)
	}
	...
	if status.CurrentMigrationVersion != 8 || len(status.AppliedVersions) != 8 || status.AppliedVersions[0] != 1 || status.AppliedVersions[1] != 2 || status.AppliedVersions[2] != 3 || status.AppliedVersions[3] != 4 || status.AppliedVersions[4] != 5 || status.AppliedVersions[5] != 6 || status.AppliedVersions[6] != 7 || status.AppliedVersions[7] != 8 || status.MinimumReaderVersion != EvidenceArchiveReaderVersion || status.MinimumWriterVersion != EvidenceReserveWriterVersion {
