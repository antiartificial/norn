...
func TestControlSchemaAdoptsLegacyRowsWithoutReplacingEvidence(t *testing.T) {
	...
	if len(migrations) != 42 || migrations[16].Version != 17 {
		t.Fatalf("control migration catalog has %d migrations, want immutable baseline through 17 and appended migrations through 42", len(migrations))
	}
	...
	if status.CurrentMigrationVersion != 42 || len(status.AppliedVersions) != 42 || status.AppliedVersions[41] != 42 || status.MinimumReaderVersion != MySQLRetainedArtifactReaderVersion || status.MinimumWriterVersion != SnapshotExportIntentWriterVersion {
