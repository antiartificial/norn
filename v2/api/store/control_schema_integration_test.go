...
func TestControlSchemaAdoptsLegacyRowsWithoutReplacingEvidence(t *testing.T) {
	...
	if len(migrations) != 41 || migrations[16].Version != 17 {
		t.Fatalf("control migration catalog has %d migrations, want immutable baseline through 17 and appended migrations through 41", len(migrations))
	}
	...
	if status.CurrentMigrationVersion != 41 || len(status.AppliedVersions) != 41 || status.AppliedVersions[40] != 41 || status.MinimumReaderVersion != MySQLRetainedArtifactReaderVersion || status.MinimumWriterVersion != SnapshotExportIntentWriterVersion {
