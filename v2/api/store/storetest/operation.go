// Package storetest holds the backend-neutral conformance suites for the Norn
// v3 control-store boundaries. Each Run* function is the shared behavioral
// contract for one boundary interface; both the PostgreSQL adapter (in
// store's own tests) and the in-memory adapter (in memstore's tests) run them,
// which is P5's second-backend acceptance mechanism. Living in a normal package
// rather than a _test.go file is what lets multiple adapter packages import and
// reuse them.
package storetest
...
// store.OperationStore — claim exclusivity, lease fencing, idempotency, terminal
// transitions and interrupted-operation recovery. newStore must return a store
// backed by an empty operation queue on each call.
func RunOperationStoreConformance(t *testing.T, newStore func(t *testing.T) store.OperationStore) {
