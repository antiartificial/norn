...
// store.DeploymentStore. newStore must return a store backed by empty deployment
// tables on each call.
func RunDeploymentStoreConformance(t *testing.T, newStore func(t *testing.T) store.DeploymentStore) {
	...
	mustInsert := func(t *testing.T, s store.DeploymentStore, d *model.Deployment) {
		t.Helper()
