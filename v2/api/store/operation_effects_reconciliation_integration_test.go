...
func TestCompletedSnapshotOperationsQueriesDurableSuccessesWithRealPostgres(t *testing.T) {
	...
	reservation := effectReservation(t, authority, "app/demo/snapshot", "snapshot-execution", claim)
	...
	identity := effect.ExecutionIdentity{Supervisor: reservation.Supervisor, SupervisorExecutionID: reservation.SupervisorExecutionID, RuntimeInstanceID: "snapshot-runtime"}
	...
	records, err := store.CompletedSnapshotOperations(ctx)
	if err != nil || len(records) != 1 || records[0].Reservation.OperationClaim.OperationID != op.ID || records[0].Execution != identity {
