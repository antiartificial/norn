...
func TestMySQLRestoreProcessKillAfterExecutingQualification(t *testing.T) {
	...
	maintenance := &database.MySQLMaintenanceCredentials{Generation: 1, RuntimeAccountHost: "%", RestoreRole: restoreRole, RestoreAccountHost: "%", RestoreCredentialRef: "secret:kill/restore", FenceRole: fenceRole, FenceCredentialRef: "secret:kill/fence", FenceAccountHost: "%"}
	...
	receipt := testMySQLSourceArtifactReceipt(t, control, stores[0], active.Revision, source.Target, path, artifact)
	testBindMySQLSourceFence(t, control, receipt)
	request := MySQLRestoreRequest{CatalogRevision: active.Revision, ProfileID: "mini", LogicalID: "kill-target", Target: target.Target, Maintenance: *target.MySQLMaintenance, Artifact: artifact, ArtifactPath: path, SourceArtifact: receipt}
