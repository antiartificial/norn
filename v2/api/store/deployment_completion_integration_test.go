...
func TestCompleteDeploymentResultIsClaimFencedAndAtomicAcrossRegions(t *testing.T) {
	...
	if _, err := pool.Exec(ctx, `INSERT INTO operations(id,kind,app,status,payload,locked_by,lock_generation,locked_until)
		VALUES($1,'app.deploy','completion-app','running','{"deploymentId":"deployment-completion"}'::jsonb,$2,$3,now()+interval '1 minute')`, claim.OperationID(), claim.OwnerID(), claim.Generation()); err != nil {
		t.Fatal(err)
	}
	...
	if err := db.CompleteDeploymentResult(ctx, claim, d, []DeploymentCompletionRegion{{Region: "east", ActiveWeight: 60}, {Region: "west", ActiveWeight: 40}}); !errors.Is(err, ErrOperationOwnershipLost) {
	...
	if err := db.CompleteDeploymentResult(ctx, claim, d, []DeploymentCompletionRegion{{Region: "east", ActiveWeight: 60}, {Region: "missing", ActiveWeight: 40}}); err == nil {
	...
	if err := db.CompleteDeploymentResult(ctx, claim, d, want); !errors.Is(err, ErrOperationOwnershipLost) {
	...
	if err := db.CompleteDeploymentResult(ctx, claim, d, want); err != nil {
		t.Fatal(err)
	}
