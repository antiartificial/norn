...
CREATE TABLE mysql_runtime_launch_reservations (
 ...
 CHECK ((state = 'reserved' AND runtime_instance_id = '' AND stop_proof IS NULL AND resolution_proof IS NULL) OR
        (state = 'launched' AND runtime_instance_id <> '' AND stop_proof IS NULL AND resolution_proof IS NULL) OR
        (state = 'needs-inspection' AND stop_proof IS NULL AND resolution_proof IS NULL) OR
        (state = 'stopped' AND runtime_instance_id <> '' AND stop_proof IS NOT NULL AND resolution_proof IS NULL) OR
        (state = 'released' AND runtime_instance_id = '' AND stop_proof IS NULL AND resolution_proof IS NOT NULL))
);
