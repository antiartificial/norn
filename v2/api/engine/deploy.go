...
func (e *Engine) deployNew(ctx context.Context, spec *model.InfraSpec, procName string, proc model.Process, imageTag string, env map[string]string, count int) error {
	for i := 0; i < count; i++ {
		...
		e.registerInstance(name, spec.App, procName, i, "service", imageTag, proc.Port, effectiveHostPort(proc))
	}
	return nil
}
...
func (e *Engine) rollingUpdate(ctx context.Context, spec *model.InfraSpec, procName string, proc model.Process, imageTag string, env map[string]string, count int) error {
	...
	// A locally published host port cannot be bound by both generations. Until
	// a native local ingress connector owns port switching, endpoint updates are
	// deliberately stop/start and therefore may have a brief interruption.
	if proc.Port > 0 {
		for _, instance := range old {
			containerStopAndRemove(ctx, instance.ContainerName, 30*time.Second)
			e.removeInstance(instance.ContainerName)
		}
		old = nil
	}
	...
	for i := 0; i < count; i++ {
		...
		e.registerInstance(tempName, spec.App, procName, i, "service", imageTag, proc.Port, effectiveHostPort(proc))
...
func (e *Engine) deployCanary(ctx context.Context, spec *model.InfraSpec, procName string, proc model.Process, imageTag string, env map[string]string) error {
	...
	for i := 0; i < count; i++ {
		...
		e.registerInstance(name, spec.App, procName, i, "canary", imageTag, proc.Port, effectiveHostPort(proc))
	}
	return nil
}
...
func (e *Engine) PromoteDeployment(ctx context.Context, appID string) error {
	...
	for procName, proc := range spec.Processes {
		...
		mergedEnv := mergeEnv(spec.Env, proc.Env, e.appEnvironment(appID))
		...
		for i := proc.Canary.Count; i < count; i++ {
			...
			e.registerInstance(name, appID, procName, i, "service", deploy.ImageTag, proc.Port, effectiveHostPort(proc))
		}
...
// RestartJob preserves the container configuration, including its private
...
func (e *Engine) ScaleJob(ctx context.Context, appID, process string, count int) error {
	...
	if count > running {
		...
		for i := running; i < count; i++ {
			...
			e.registerInstance(name, appID, process, i, "service", imageTag, proc.Port, effectiveHostPort(proc))
		}
...
func (e *Engine) registerInstance(name, app, process string, replica int, kind, imageTag string, port, hostPort int) {
	e.mu.Lock()
	defer e.mu.Unlock()
...
func (e *Engine) RunBatch(ctx context.Context, spec *model.InfraSpec, procName string, proc model.Process, imageTag string, env map[string]string) (string, error) {
	...
	e.registerInstance(name, spec.App, procName, 0, "batch", imageTag, proc.Port, effectiveHostPort(proc))
	return name, nil
}
...
func buildRunOpts(name string, spec *model.InfraSpec, procName string, proc model.Process, imageTag string, env map[string]string) RunOpts {
	opts := RunOpts{
		...
		Env:      env,
		Port:     proc.Port,
		HostPort: effectiveHostPort(proc),
	}
	if proc.Resources != nil {
		opts.CPUs = (proc.Resources.CPU + 999) / 1000 // approximate requested MHz as whole cores
...
func effectiveHostPort(proc model.Process) int {
	if proc.HostPort > 0 {
		return proc.HostPort
	}
	return proc.Port
}
