...
func (w *EngineWatcher) checkServiceHealth(ctx context.Context, spec *model.InfraSpec) {
	for processName := range spec.Processes {
		...
		_, err = w.beacon.Emit(ctx, model.BeaconEvent{
			...
			Severity:  severity,
			Title:     title,
			Body:      fmt.Sprintf("Service %s changed from %s to %s.", serviceName, engineEmptyState(previous), state),
...
func engineEmptyState(state string) string {
	if state == "" {
		return "unknown"
	}
	return state
...
const engineCronMissedGracePeriod = 5 * time.Minute

func engineCronEvaluationLocation(spec *model.InfraSpec, proc model.Process, info *engine.CronJobInfo) *time.Location {
	timezone := ""
	if info != nil {
...
func (w *EngineWatcher) checkCronMissedRuns(ctx context.Context, spec *model.InfraSpec) {
	for process, proc := range spec.Processes {
		...
		location := engineCronEvaluationLocation(spec, proc, info)
		...
		deadline := expectedNextRun.Add(engineCronMissedGracePeriod)
		...
		_, emitErr := w.beacon.Emit(ctx, model.BeaconEvent{
			...
			Metadata: map[string]interface{}{
				...
				"gracePeriod":    engineCronMissedGracePeriod.String(),
