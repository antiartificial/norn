...
scripts/lab secrets log <lab>      # list retained Spaces versions
scripts/lab secrets rollback <lab> <version-id>  # restore a version as new latest
...
Before relying on this recovery path or a newly added recipient, run
`scripts/lab secrets qualify <lab>` with approved Spaces credentials. It writes
only a timestamped synthetic ciphertext under `qualifications/<lab>/` and proves
version retention, rollback promotion, and recipient rotation. It never reads or
changes the lab's real secret object or recipient list.
