...
var runtimeCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		connectorDot := style.DotHealthy
		if !connector.Available {
			connectorDot = style.DotUnhealthy
		}
		fmt.Println(style.Bold.Render("  workload connector"))
		fmt.Printf("    %s %s\n", connectorDot, style.Bold.Render(connector.Name))
		fmt.Printf("      scheduler: %s  discovery: %s  runtime: %s\n", connector.Scheduler, connector.Discovery, connector.Runtime)
		if len(connector.Limitations) > 0 {
			fmt.Printf("      limits: %s\n", style.DimText.Render(strings.Join(connector.Limitations, "; ")))
		}
		fmt.Println()
