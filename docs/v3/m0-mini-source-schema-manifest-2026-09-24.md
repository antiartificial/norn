...
The local PostgreSQL integration test applies migrations 1–16 to a synthetic unversioned baseline and checks selected legacy rows. It is distinct from the prior exact Mini schema-only restore, which reached migration 13. The next isolated rehearsal must restore representative Mini data with all workers, webhooks, cron, provider mutation, and production egress disabled; run the exact candidate through migration 16; compare stable IDs, receipts, schemas, and application identity; and exercise the supported rollback window. Old-worker compatibility and source rollback after writer contract 13 are unproven. M0 still needs app/job/route/volume/database ownership, representative fixtures, measured budgets, ADR decisions, and named acceptance.
