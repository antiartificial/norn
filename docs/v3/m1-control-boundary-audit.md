...
| `handler/cron.go:90-365` | Force, stop, or resubmit periodic jobs and then separately update cron state | Make the desired cron state and operation one atomic aggregate; execute the Nomad change from a claim and reconcile ambiguous submissions by evaluation/job identity. |
