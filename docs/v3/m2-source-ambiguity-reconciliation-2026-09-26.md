...
Migration 41 widens the history checkpoint constraint to include `stop-proved` and `lock-proved` while retaining the same 5/31 compatibility floor. The failed successor is eligible only while it still owns the source row and runtime fence at one of those proved checkpoints. The new operation freshly observes the exact stopped job, and also the locked/drained account for `lock-proved`, before another atomic transfer. Every predecessor remains inspectable through its signed proof chain.

Migration 42 adds `stage-intended` to that constraint at the same 5/31 compatibility floor. A failed, still-fenced stage predecessor may be replaced only after the new signed successor freshly proves the stopped job and locked/drained account. The transfer clears the prior unsigned stage intent and starts a new SQL dump under the successor operation. A local file left by a killed process has no signed stage receipt and is never adopted; it remains for deliberate operator cleanup. `publish-intended` remains an open implementation boundary; its [successor contract](m2-publish-intended-successor-contract-2026-09-26.md) records the proposed proof and qualification path.
...
5. The successor runs only the effects after the proved checkpoint: account lock after stop proof, or fresh staging after account-lock proof. Staging, retention, and terminal success use the successor's signed identity and receipts. A failed successor at `stop-proved`, `lock-proved`, or `stage-intended` can itself be named by a new signed successor after fresh observation. An unsigned predecessor dump cannot qualify as a stage receipt. An ambiguous publication effect remains fenced for separate recovery design.
...
- The compiled CLI fixture exercises `stop-intended` with the Nomad stop effect completed and its response deliberately lost, then signs a successor, proves the stopped job, transfers the fence, locks the account, retains the artifact, restores it, and serves WordPress from the restored target. PostgreSQL qualification injects a failure during the transfer after the source-intent update and verifies rollback of intent, fence, and proof at both checkpoints; same-claim replay after a committed transfer uses durable proof without another external observation. PostgreSQL tests also expire a transferred successor at `stop-proved` and `lock-proved`, then verify a second signed transfer and inspection of both predecessors. Disposable build-tag hooks pause the compiled command immediately before or after transaction commit. The fixture kills it at each boundary, expires its claim, and runs a new signed successor through retention, restore, and WordPress startup. Before commit, inspection proves the predecessor still owns the fenced source and no proof row survived. After commit, inspection verifies the signed transfer and the new successor names the failed transferred operation. The normal binary compiles no-op hooks. Separate-node/provider and managed-MySQL rehearsals remain release gates.
- A separate disposable hook pauses the compiled command after `mysqldump` completes but before the stage receipt commits. The WordPress fixture kills that process, verifies `stage-intended` has no signed receipt, expires its claim, and uses another signed successor to create and retain a fresh stage before restore and application startup. The predecessor's unsigned local dump is not adopted. The file is left for operator cleanup; no automatic deletion of ambiguous local bytes is authorized.
...
For the disposable numeric loopback S3 emulator only, add `--s3-loopback-http`. The command keeps the source fenced on any indeterminate observation or later effect failure; inspect the exact predecessor and successor before deciding the next action. If a transferred successor fails at `stop-proved`, `lock-proved`, or `stage-intended`, name that failed operation as `--prior-source-operation-id` and use a new request key. At `stage-intended`, inspect and account for any unsigned local SQL file separately; the next command creates a new dump. Do not point the new command at its older, already reconciled predecessor.
