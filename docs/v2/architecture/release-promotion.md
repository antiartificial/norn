...
admission. Staging may deliberately use `NORN_PROFILE=production`; production
is refused at startup unless both `NORN_ENVIRONMENT=production` and
`NORN_PROFILE=production` are set.
...

{
  ...
  "candidate": {
    ...
    "attestation": {
      "provenanceURI": "<OCI provenance URI>",
      "sbomURI": "<OCI SBOM URI>",
...
The v1 GitHub attestation path intentionally supports **public** caller
repositories only. The reusable workflow fails before any release work unless
the caller's GitHub context reports `repository.visibility=public`, and Norn
also enforces the signed GitHub OIDC `repository_visibility` claim. Private and
internal repositories use GitHub private Sigstore attestations, which Norn does
not inspect in this path. They remain unsupported until a separate read-only
Release Attestation GitHub App verifier is introduced; do not try to bypass this
boundary with a GitHub token or a copied attestation.
